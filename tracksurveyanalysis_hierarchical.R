...
sim.fun <- function(n.sites){
  ...
  #  Detection probability of each method
  P <- vector("numeric")

  # Detection probability of tracks
  P[1] <- 0.5
  # Detection probability of genetic sample
  P[2] <- 0.3
  ...
  sink("model_multinomial_multisp_sim_trackh.txt")
  cat("
      model{
      #  Priors
      ...
      #  Realizations
      #  Number counted with each survey method 
      y[i,1] ~ dbin(cp[i,1], N[site[i],sp[i]])
  ...
  out <- jags.parallel(data=names(data), inits, parms, "model_multinomial_multisp_sim_trackh.txt", 3, 20000, 1000, 1)
...
##################### Plot #############################
# N mean absolute percent error (species 1)
sp1 <-as.data.frame(sim.out[[1]][[1]]$N[,9:12])
colnames(sp1)<-(c("1","2","3","4"))
sp1<-melt(sp1)
colnames(sp1)<-c("sp","predN")
sp1$index <-1:nrow(sp1)

ggplot(data=sp1, aes(x=index, y=predN, fill= as.factor(sp))) + 
  ...
  ggtitle("Mean absolute percent error species 1")